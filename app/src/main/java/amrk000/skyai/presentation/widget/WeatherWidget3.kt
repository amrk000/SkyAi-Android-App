...
@AndroidEntryPoint
class WeatherWidget3 : AppWidgetProvider() {
    @Inject
    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        val views = RemoteViews(context.packageName, R.layout.weather_widget3)

        //Open App on Widget Click
        val intent = Intent(context, MainActivity::class.java)
        val pendingIntent = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_IMMUTABLE)
        views.setOnClickPendingIntent(R.id.widget3Layout, pendingIntent)
    ...
    fun renderWeatherData(context: Context, weatherData : WeatherData, views: RemoteViews){
        ...
        if(timeLines!= null) {
            ...
            views.setImageViewResource(R.id.widget3StatusIcon, statusIcon)
            views.setTextViewText(R.id.widget3Status, status)
            views.setTextViewText(R.id.widget3TempUnit, unites.temperatureUnit)
            views.setTextViewText(R.id.widget3Temp, temperature)
